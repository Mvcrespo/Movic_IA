...
async function interpretWithOllama(
  ...
  if (!response.ok) {
    if (env.ollamaTimingLogs) {
      console.log(
        `[llm-service] Ollama /api/chat falhou em ${Date.now() - startedAt}ms (${response.status} ${response.statusText})`
      );
    }
    throw new Error(
  ...
  if (env.ollamaTimingLogs) {
    console.log(
      `[llm-service] Ollama /api/chat concluido em ${Date.now() - startedAt}ms`
    );
  }
...
async function ensureModelAvailableOnce(): Promise<void> {
  ...
  if (!pullResponse.ok) {
    throw new Error(
      `Falhou o pull do modelo ${env.ollamaModel}: ${pullResponse.status} ${pullResponse.statusText}`
    );
  }
...
function safeJsonParse(value: string): LlmInterpretation | null {
  try {
    return JSON.parse(value) as LlmInterpretation;
  } catch {
    return null;
  }
}
