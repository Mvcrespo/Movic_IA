...
const ollamaBaseUrl =
  process.env.VALIDATOR_OLLAMA_BASE_URL ?? process.env.OLLAMA_BASE_URL ?? "http://ollama:11434";
const ollamaApiKey =
  process.env.VALIDATOR_OLLAMA_API_KEY ?? process.env.OLLAMA_API_KEY ?? "";

const env = {
  port: Number(process.env.VALIDATOR_SERVICE_PORT ?? "8005"),
  ollamaBaseUrl,
  model:
    process.env.VALIDATOR_OLLAMA_MODEL ??
    process.env.VALIDATOR_MODEL ??
    process.env.OLLAMA_MODEL ??
    "qwen2.5:3b",
  ollamaApiKey,
  autoPull: resolveBoolean(
    process.env.VALIDATOR_OLLAMA_AUTO_PULL ?? process.env.VALIDATOR_AUTO_PULL,
  ...
  for (let attempt = 1; attempt <= maxSchemaRepairAttempts; attempt += 1) {
    ...
    if (!response.ok) {
      ...
      if (env.ollamaTimingLogs) {
        console.log(
          `[validator-service] Ollama /api/chat falhou em ${Date.now() - startedAt}ms (${response.status} ${response.statusText}${errorDetail ? ` | ${errorDetail}` : ""})`
        );
      }
      throw new Error(
...
function safeJsonParse(value: string): ValidationResponse | null {
  for (const candidate of getJsonCandidates(value)) {
    try {
      return JSON.parse(candidate) as ValidationResponse;
    } catch {
      continue;
    }
  }
